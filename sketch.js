...
function setup(){
 createCanvas(windowWidth,windowHeight);
  ...
  clebinho = createSprite(50,height-50,20,20);
  ...
  chaoinv = createSprite(width/2,height-15,width,15)
...
function draw(){
  ...
  fill('black');
  textSize(20);
  text(ponto,width-100,height-370);
  ...
  if (modo === INICIAR){
    ...
    //pulo do clebinho
    if (touches.length > 0 && clebinho.isTouching(chao)){
...
function nuvens(){
  ...
  nuvem = createSprite(width+10,50,10,10);
    
    nuvem.y = Math.round(random(height-160,height-80));
...
function obstaculo(){
    ...
    if(frameCount % 60 === 0){
      ...
      obst = createSprite(width+50,height-30,20,10);
